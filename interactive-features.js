...
class InteractiveFeatures {
  ...
  constructor() {
    ...
    this.DRAG_Z_INDEX = 1000;
    this.HIGHLIGHT_THROTTLE = 100;
    this.SCORE_INCREMENT = 10;
    this.SUCCESS_DELAY = 1000;
    this.FEEDBACK_DURATION = 500;
    this.PARTICLE_COUNT = 20;
    this.PARTICLE_SPREAD = 200;
    this.PARTICLE_ANIMATION_DURATION = 1000;
    this.SCORE_ANIMATION_DURATION = 300;
    this.GAME_COMPLETE_DELAY = 2000;
    this.PASSING_SCORE_PERCENTAGE = 80;
    this.TOTAL_QUESTIONS = 5;
    this.CELEBRATION_PARTICLE_COLORS = [
      "#FFD700",
      "#FF69B4",
      "#87CEEB",
      "#98FB98",
    ...
    this.dragAndDropInitialized = false; // Flag to prevent duplicate listener setup
  ...
  /**
   * Setup drag and drop functionality (prevents duplicate initialization)
   */
  setupDragAndDrop() {
    // Only initialize once per instance
    if (this.dragAndDropInitialized) {
      this.initializeDragAndDrop(); // Still call on mode switches
      return;
    }
   ...
   * Uses element property flags to avoid duplicate event listener attachment
   */
  initializeDragAndDrop() {
    ...
    if (this.debugMode) {
      console.log(
        `Initializing drag and drop: ${draggables.length} draggables, ${dropZones.length} drop zones`
      );
    }

    draggables.forEach((draggable) => {
      // Skip if handlers already attached (using data attribute flag)
      if (draggable.dataset.dragInitialized === "true") return;

      // Create bound event handlers
      const handleDragStart = (e) => this.handleDragStart(e, draggable);
    ...
    dropZones.forEach((zone) => {
      ...
      if (zone.dataset.dropInitialized === "true") return;

      // Create bound event handlers
      const handleDragOver = (e) => this.handleDragOver(e);
      const handleDrop = (e) => this.handleDrop(e, zone);
      const handleTouchMove = (e) => this.handleDragMove(e);
      const handleTouchEnd = (e) => this.handleTouchEnd(e, zone);
    ...
    // Global mouse/touch events (only attach once per instance)
    if (!this._globalDragEventsAttached) {
      const handleDragMove = (e) => this.handleDragMove(e);
      const handleDragEnd = (e) => this.handleDragEnd(e);
      ...
      this._globalDragEventsAttached = true;
    }
  }
  ...
  handleDragStart(dragEvent, draggableElement) {
    ...
    if (dragEvent.type.includes("touch")) {
      cloneElement.style.left =
        dragEvent.touches[0].clientX - elementBoundingRect.width / 2 + "px";
      cloneElement.style.top =
        dragEvent.touches[0].clientY - elementBoundingRect.height / 2 + "px";
    } else {
      cloneElement.style.left =
        dragEvent.clientX - elementBoundingRect.width / 2 + "px";
      cloneElement.style.top =
        dragEvent.clientY - elementBoundingRect.height / 2 + "px";
    }
  ...
  handleDragMove(dragEvent) {
    ...
    const cursorX = dragEvent.type.includes("touch")
      ? dragEvent.touches[0].clientX
      : dragEvent.clientX;
    const cursorY = dragEvent.type.includes("touch")
    ...
    if (
      !this.lastHighlightCheck ||
      currentTimestamp - this.lastHighlightCheck > this.HIGHLIGHT_THROTTLE
    ) {
      ...
      if (!this.dropZonesCache) {
        this.dropZonesCache = Array.from(
      ...
      if (
        targetDropZone &&
        this.isValidDrop(this.draggedElement, targetDropZone)
      ) {
  ...
  handleDragEnd(dragEvent) {
    ...
    const finalCursorX = dragEvent.type.includes("touch")
      ? dragEvent.changedTouches[0].clientX
      : dragEvent.clientX;
    const finalCursorY = dragEvent.type.includes("touch")
      ? dragEvent.changedTouches[0].clientY
      : dragEvent.clientY;
  ...
  createCelebrationParticles(targetElement) {
    ...
    for (
      ...
      particleElement.style.top =
        elementRect.top + elementRect.height / 2 + "px";
      particleElement.style.setProperty(
        "--random-x",
        (Math.random() - 0.5) * this.PARTICLE_SPREAD + "px"
      );
      particleElement.style.setProperty(
        "--random-y",
        (Math.random() - 0.5) * this.PARTICLE_SPREAD + "px"
      ...
      particleElement.style.backgroundColor =
        this.CELEBRATION_PARTICLE_COLORS[
          Math.floor(Math.random() * this.CELEBRATION_PARTICLE_COLORS.length)
        ];
      ...
      setTimeout(
        ...
        this.PARTICLE_ANIMATION_DURATION
      );
    }
  }
  ...
  updateScore(points) {
    ...
    if (soundScoreElement) {
      ...
      setTimeout(
        () => soundScoreElement.classList.remove("score-bump"),
        this.SCORE_ANIMATION_DURATION
      );
    }
  }
  ...
  start() {
    ...
    gameContainer.innerHTML = `
            ...
            <div class="animals-pool">
                ${Object.entries(this.habitats)
                  .map(([habitatName, animalList]) =>
                    animalList
                      .map(
                        (animalName) => `
...
class SoundMatchingGame {
  ...
  generateSoundGameOptions() {
    ...
    const incorrectAnimals = allAvailableAnimals.filter(
      (animalName) => animalName !== this.currentAnimal
    );
    const selectedIncorrectAnimals = incorrectAnimals
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    // Combine and shuffle
    const allOptionsForRound = [
      this.currentAnimal,
      ...selectedIncorrectAnimals,
    ];
  ...
  checkAnswer(selectedAnimalName) {
    ...
    if (isCorrectAnswer) {
      this.correctAnswers++;
      window.interactiveFeatures.updateScore(
        window.interactiveFeatures.SCORE_INCREMENT
      );
    ...
    allOptionButtons.forEach((optionButton) => {
      ...
      if (optionButton.textContent.includes(this.currentAnimal)) {
        ...
        optionButton.textContent.includes(selectedAnimalName) &&
        !isCorrectAnswer
      ) {
  ...
  showCompletionScreen() {
    const gameContainerElement = document.querySelector(
      ".sound-game-container"
    );
    const scorePercentage = Math.round(
      (this.correctAnswers / window.interactiveFeatures.TOTAL_QUESTIONS) * 100
    );

    let completionMessage = "";
    if (
      scorePercentage >= window.interactiveFeatures.PASSING_SCORE_PERCENTAGE
    ) {
